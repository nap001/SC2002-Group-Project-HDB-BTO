    ...
    private void createProject(Scanner scanner) {
        ...
        while (openDate == null) {
            try {
                ...
                openDate = LocalDate.parse(scanner.nextLine().trim());
        ...
        while (closeDate == null || closeDate.isBefore(openDate)) {
            try {
                System.out.print("Enter application close date (YYYY-MM-DD): ");
                closeDate = LocalDate.parse(scanner.nextLine().trim());
                if (closeDate.isBefore(openDate)) {
                    System.out.println("Close date must be on or after open date.");
                }
            } catch (DateTimeParseException e) {
                System.out.println("Invalid date format. Please use YYYY-MM-DD.");
            }
        ...
        while (visible == null) {
            ...
            if (input.equals("true") || input.equals("false")) {
                visible = Boolean.parseBoolean(input);
            } else {
        ...
        for (FlatType flatType : FlatType.values()) {
            ...
            while (unitCount < 0) {
                ...
                try {
                    unitCount = Integer.parseInt(scanner.nextLine().trim());
                    if (unitCount < 0) {
                        System.out.println("Unit count must be non-negative.");
                    }
                } catch (NumberFormatException e) {
                    System.out.println("Invalid number. Please enter a non-negative integer.");
                }
            }
            unitCountMap.put(flatType, unitCount);
            ...
            priceMap.put(flatType, price);
        }

        manager.createProject(iProjectManagementControl, iProjectQueryControl,
                projectName, neighbourhood, openDate, closeDate, visible, officerSlots, unitCountMap, priceMap);
    }
